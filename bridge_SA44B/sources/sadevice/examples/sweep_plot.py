...
def sweep():
    ...
    # Configure device
    sa_config_center_span(handle,  106.6e6, 1.0e6)
...
def sweepTest():
    ...
    # Configure device
    sa_config_center_span(handle,  106.6e6, span)
    value=(span/100.0)/5.
    leng=(10**(len(str(value))-3))
    value=math.floor(value/leng)*5*leng
    
    sa_config_level(handle, -30.0)
    sa_config_sweep_coupling(handle, value, 10e3, 0)
    sa_config_acquisition(handle, SA_MIN_MAX, SA_LOG_SCALE)
    ...
    # Get sweep
    sweep_max = sa_get_sweep_32f(handle)["max"]
    print(str(sweep_length)+" "+str(type(sweep_max[0])))
